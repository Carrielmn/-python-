...
import urllib.request as r#导入联网工具包，命令为r
url='https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=2018-07-02&leftTicketDTO.from_station=CQW&leftTicketDTO.to_station=NJH&purpose_codes=ADULT'
data=r.urlopen(url).read().decode('utf-8')
#讲str类型转换为dict
import json
data=json.loads(data)
...
p='  '
title='车次{}出发站/到达站{}出发时间/到达时间{}历时{}商务座/特等座{}一等座{}二等座{}高级软卧{}软卧{}动卧{}硬卧{}软座{}硬座{}无座{}其他{}备注'.format(p,p,p,p,p,p,p,p,p,p,p,p,p,p,p)
title=title.split(p)
for j in title:
    print(j.center(11),end='')
len([p,p,p,p,p,p,p,p,p,p,p,p,p,p,p])
print()  
for i in range(0,16):
    s=data[i]
    ls=s.split('|')
    ls1=[ls[3],[ls[6],ls[7]],[ls[8],ls[9]],ls[10],ls[32],ls[31],ls[30],'--',ls[23],ls[33],ls[28],'--',ls[29],ls[26],ls[22],ls[1]]
    for i in ls1:
        print(str(i).center(14).replace('[','').replace(']','').replace('CUW','重庆北').replace('CXW','重庆西').replace('NJH','南京').replace('NKH','南京南'),end='')
